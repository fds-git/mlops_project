...
- экспорт обученной модели в S3
- развертывание сервиса с обновленной моделью

Важно: ВМ с AirFlow, MLFlow и кластер Data Proc должны находиться в одной сети и иметь одну и ту же группу безопасности
  
На кластере kubernetes развернуто приложение на FastAPI, которое реализует сервис модели

Настроен CI/CD в GitHub
